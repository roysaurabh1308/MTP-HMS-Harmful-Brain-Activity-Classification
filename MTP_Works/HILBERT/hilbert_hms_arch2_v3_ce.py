...
def createModel(ip_shape):
    saved_model_path = '/home/m1/23CS60R76/MTP_Works/BestSavedModels/H_HMS_model5_arch2_v3.keras'
    ...
    if os.path.isfile(saved_model_path):
        ...
        inputs = keras.Input(shape=ip_shape)

        # Initial Convolutional Layers
        x = layers.Conv2D(filters=64, kernel_size=(2, 4), padding='same', activation='relu')(inputs)
        x = layers.Conv2D(filters=64, kernel_size=(2, 4), strides=(1, 2), padding='same', activation='relu')(x)
        x = layers.MaxPooling2D((1, 2))(x)

        # Hilbert Layer and Concatenation
        hilbert_output = HilbertLayer()(x)
        concatenated = layers.Concatenate()([x, hilbert_output])

        # Continue with Convolutional Layers
        x = layers.Conv2D(filters=128, kernel_size=(2, 4), padding='same', activation='relu')(concatenated)
        x = layers.Conv2D(filters=128, kernel_size=(2, 4), strides=(1, 2), padding='same', activation='relu')(x)
        x = layers.MaxPooling2D((2, 2))(x)

        x = layers.Conv2D(filters=256, kernel_size=(4, 4), padding='same', activation='relu')(x)
        x = layers.Conv2D(filters=256, kernel_size=(4, 4), strides=(1, 2), padding='same', activation='relu')(x)
        x = layers.MaxPooling2D((1, 2))(x)
...
checkpoint_filepath = '/home/m1/23CS60R76/MTP_Works/BestSavedModels/H_HMS_model5_v3.keras'
...
# Saving the figure.
plt.savefig("hilbert_training_history_A2_1.jpg")
