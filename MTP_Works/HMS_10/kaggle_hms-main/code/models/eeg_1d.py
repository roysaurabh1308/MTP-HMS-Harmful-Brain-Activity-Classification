...
class EEGNet(nn.Module):
    ...
    def __init__(self, kernels, in_channels=20, fixed_kernel_size=17):
...
if __name__ == '__main__':
    model = EEGNet(kernels=[3,5,7,9], in_channels=16, fixed_kernel_size=5).cuda()
    print('Number of parameters: {}'.format(
        sum([p.data.nelement() for p in model.parameters()])))
    x = torch.rand((2, 16, 2000)).cuda()
    # x = x[:, ::5,:]
    print(x.shape)
    print(model(x).shape)